...
-- Auto-starts on execute. Fast scan; hops to lowest-pop public server (max 1 player) if no Alternate boxes.
...
local function activateCloak()
    ...
    for _, p in ipairs(char:GetDescendants()) do
        if
            (p:IsA("BasePart") and p.Name ~= "HumanoidRootPart")
            or p:IsA("Decal")
            or p:IsA("Texture")
        then
            p.Transparency = 1
...
local function findBlock()
    ...
    for _, m in ipairs(folder:GetChildren()) do
        if m:IsA("Model") and not m:GetAttribute("Carrying") then
            ...
            if name:find("Alternate", 1, true) and TARGET.Alternative then
                local part =
                    m.PrimaryPart
                    ...
                    if not bestV or v > bestV then
                        ...
                        for _, d in ipairs(m:GetDescendants()) do
                            if d:IsA("ProximityPrompt") and d.Enabled then
                                ...
                                local at =
                                    tostring(d.ActionText or ""):lower()
                        ...
                        best = {
                            rarity = "Alternative",
                            id = "1263",
                            luckyBlockName = "Alternate Lucky Block",
                            part = part,
                            prompt = prompt,
                            model = m
                        }
...
local function findLowPopJobId(placeId)
    ...
    while pages < 5 do
        ...
        if
            bestId
            and bestPlaying <= MAX_SERVER_PLAYERS
        then
            return bestId, bestPlaying
        end
...
pcall(function()
    local o =
        PG:FindFirstChild("AlternateStealer")
        or PG:FindFirstChild("JapanStealer")
        or PG:FindFirstChild("JIStealer")
...
statusLbl.Text = "Auto-run | scanning Alternate..."
...
local function setOn(on)
    ...
    if on then
        ...
        statusLbl.Text =
            "Scanning Alternate Lucky Blocks..."
    else
        busy = false
...
task.spawn(function()
    ...
    while true do
        ...
        if enabled and not busy then
            ...
            local alternativeCount =
                countAlternativeBlocks()
            ...
            if
                (not prompt or not prompt.Parent)
                and b.model
            then
...
print(
    "[AlternateStealer] Auto-run ON | hop to <=1 player servers if no Alternate after",
    EMPTY_SCANS_BEFORE_HOP,
    "empty scans"
)