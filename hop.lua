...
local function findBlock()
    ...
    for _, m in ipairs(folder:GetChildren()) do
        if m:IsA("Model") and not m:GetAttribute("Carrying") then
            local name = m.Name
            if name:find("Japan", 1, true) and TARGET.Japan then
                ...
                if part then
                    ...
                    if not bestV or v > bestV then
                        ...
                        bestV = v
                        best = { rarity = "Japan", part = part, prompt = prompt, model = m }
                    end
                end
            end
        end
...
local function countJapanBlocks()
    local live = workspace:FindFirstChild("Live")
    local folder = live and live:FindFirstChild("Slimes")
    ...
    for _, m in ipairs(folder:GetChildren()) do
        if m:IsA("Model")
            ...
            and tostring(m.Name):find("Japan", 1, true)
        then
            n += 1
        end
    end
...
statusLbl.Text = "Auto-run | scanning Japan..."
...
local function setOn(on)
    ...
    if on then
        ...
        countLbl.Text = "Collected: 0"
        timeLbl.Text = "Time: 00:00"
        statusLbl.Text = "Scanning Japan boxes..."
    else
        busy = false
...
task.spawn(function()
    ...
    while true do
        ...
        if enabled and not busy then
            ...
            local japanCount = countJapanBlocks()
            if japanCount <= 0 then
                emptyScans += 1
                statusLbl.Text = string.format(
                    "No Japan (%d/%d) — will hop",
                    emptyScans,
            ...
            if not b then
                ...
                statusLbl.Text = "Japan seen — retargeting..."
                busy = false
                task.wait(0.12)
                continue
            end

            statusLbl.Text = "Japan found — stealing..."
            activateCloak()
            task.wait(0.2)
