-- Minimal Alternative-only Stealer + timer + count
-- Auto-starts on execute. Fast scan; hops to lowest-pop public server (max 1 player) if no Alternative boxes.
local Players = game:GetService("Players")
local TeleportService = game:GetService("TeleportService")
local LP = Players.LocalPlayer
local PG = LP:WaitForChild("PlayerGui")

local TARGET = { Alternative = true }   -- <-- changed from Japan
...
local function isAlternativeBlock(name)
	name = tostring(name or "")
	return name:find("Alternate", 1, true) or name:find("Alternative", 1, true)
end
...
local function findBlock()
	...
	for _, m in ipairs(folder:GetChildren()) do
		if m:IsA("Model") and not m:GetAttribute("Carrying") then
			if isAlternativeBlock(m.Name) and TARGET.Alternative then
				...
				if part then
					local v = tonumber(m:GetAttribute("Value"))
						...
						or 7e8   -- high default for Alternative tier
...
local function countAlternativeBlocks()
	local live = workspace:FindFirstChild("Live")
	local folder = live and live:FindFirstChild("Slimes")
...
-- GUI
pcall(function()
	local o = PG:FindFirstChild("JapanStealer") or PG:FindFirstChild("JIStealer") or PG:FindFirstChild("AltStealer")
...
statusLbl.Text = "Auto-run | scanning Alternative..."
...
local function setOn(on)
	...
	if on then
		...
		countLbl.Text = "Collected: 0"
		timeLbl.Text = "Time: 00:00"
		statusLbl.Text = "Scanning Alternative boxes..."
	else
		busy = false
...
task.spawn(function()
	...
	while true do
		...
		if enabled and not busy then
			...
			local altCount = countAlternativeBlocks()
			if altCount <= 0 then
				emptyScans += 1
				statusLbl.Text = string.format(
					"No Alternative (%d/%d) — will hop",
					emptyScans,
			...
			local b = findBlock()
			if not b then
				statusLbl.Text = "Alternative seen — retargeting..."
				busy = false
				task.wait(0.12)
				continue
			end

			statusLbl.Text = "Alternative found — stealing..."
			activateCloak()
			task.wait(0.2)
...
print("[AltStealer] Auto-run ON | targeting Alternative/Alternate Lucky Blocks | hop after", EMPTY_SCANS_BEFORE_HOP, "empty scans")