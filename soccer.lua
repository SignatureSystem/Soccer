--[[
    ...
    Features:
    1. Auto Collect - Collects earnings from pads
    2. Auto Upgrade - Upgrades slimes (cheapest first)
    3. Lucky Block - ONLY STEALS JAPAN AND ICONS
       • Activates Invisibility Cloak before teleport
       • Teleports 1 stud underground
       • Holds position with BodyVelocity
]]
...
-- Rarity values from game (updated)
local RARITY_VALUE = {
    ["Japan"] = 7000000,
    ["Icons"] = 5000000,
    ["Spain"] = 2500000,
...
-- ============================================
-- SECTION 3: REMOTE FINDING (UPDATED)
-- ============================================

local function findRemote(namePart, isFunction)
    local class = isFunction and "RemoteFunction" or "RemoteEvent"
    
    -- Check ReplicatedStorage first (original location)
    ...
    -- Check Network/Remotes (new location)
...
local function getStandInfo(stand)
    ...
    -- Try to get data from _Lib
    if _Lib and _Lib.Data and _Lib.Database then
        local data = _Lib.Data:Get()
        ...
        if not sellPrice and slimeId and _Lib.Database.Slimes then
    ...
    local function scan(bag)
        ...
        for _, item in ipairs(bag:GetChildren()) do
            if item:IsA("Tool") then
                ...
                if n:find("invisibility") or n:find("cloak") or n:find("invis") or n:find("phantom") then
                    return item
                end
            end
        end
...
-- ============================================
-- SECTION 7: LUCKY BLOCK LOGIC (UPDATED)
-- ============================================
...
local function getRarityValueByName(name)
    ...
    for _, model in ipairs(slimes:GetChildren()) do
        if model:IsA("Model") then
            ...
            if slimeId or string.find(model.Name, "Lucky Block", 1, true) or string.find(model.Name, "Slime", 1, true) then
                ...
                local _, rarity = getRarityValueByName(model.Name)
...
-- UPDATED: More robust prompt triggering
local function attemptSteal(prompt)
    if not prompt then return false end
    
    local holdDuration = prompt.HoldDuration or 0
    
    -- Method 1: fireproximityprompt (if available)
    ...
    -- Method 2: Simulate hold (most reliable)
    local success, err = pcall(function()
    ...
    while true do
        if upgradeEnabled and UpgradeRemote then
            ...
            if upgraded == 0 and upgradeEnabled then
                if not StatusLabel.Text:find("Lucky") and not StatusLabel.Text:find("Collect") then
...
-- ============================================
-- SECTION 12: LUCKY BLOCK LOOP (UPDATED)
-- ============================================
...
-- ============================================
-- SECTION 13: EMERGENCY FUNCTIONS
-- ============================================
...
-- Safety: Auto-disable if disconnected
LocalPlayer.CharacterAdded:Connect(function()
    if luckyEnabled then
        luckyBlockBusy = false
        deactivateCloak()
