...
-- Auto Upgrade rarity multi-select.
-- Default = every rarity checked, preserving the old "All" behavior.
local selectedUpgradeRarities = {}
for _, rarityName in ipairs(ALL_RARITIES) do
    selectedUpgradeRarities[rarityName] = true
end
...
-- Auto Upgrade mutation multi-select.
-- "Common" means truly NO mutation (no base mutation + no event mutation).
-- Default = every mutation option checked, preserving the old "All" behavior.
local selectedUpgradeMutations = {}
for _, mutationName in ipairs(UPGRADE_MUTATION_OPTIONS) do
    if mutationName ~= "All" then
        selectedUpgradeMutations[mutationName] = true
    end
end
...
UpgradeRarityDropList.ZIndex = 250
...
UpgradeMutationDropList.ZIndex = 255
...
local function countUpgradeSelections(selectedSet, options)
    ...
    for _, option in ipairs(options) do
        if option ~= "All" then
            totalCount += 1
            if selectedSet[option] == true then
                selectedCount += 1
            end
        end
    end
...
local function allUpgradeRaritiesSelected()
    local selectedCount, totalCount =
        countUpgradeSelections(
            selectedUpgradeRarities,
            UPGRADE_RARITY_OPTIONS
        )

    return totalCount > 0 and selectedCount == totalCount
end

local function allUpgradeMutationsSelected()
    local selectedCount, totalCount =
        countUpgradeSelections(
            selectedUpgradeMutations,
            UPGRADE_MUTATION_OPTIONS
        )

    return totalCount > 0 and selectedCount == totalCount
end

local function upgradeRaritySelectionLabel()
    local selectedCount, totalCount =
        countUpgradeSelections(
            selectedUpgradeRarities,
            UPGRADE_RARITY_OPTIONS
    ...
    if selectedCount == totalCount and totalCount > 0 then
        return "All"
    end
    ...
    if selectedCount == 1 then
        for _, rarityName in ipairs(UPGRADE_RARITY_OPTIONS) do
            if rarityName ~= "All"
                and selectedUpgradeRarities[rarityName] == true
            then
                return upgradeRarityDisplayName(rarityName)
            end
        end
    end
...
local function upgradeMutationSelectionLabel()
    local selectedCount, totalCount =
        countUpgradeSelections(
            selectedUpgradeMutations,
            UPGRADE_MUTATION_OPTIONS
    ...
    if selectedCount == 1 then
        for _, mutationName in ipairs(UPGRADE_MUTATION_OPTIONS) do
            if mutationName ~= "All"
                and selectedUpgradeMutations[mutationName] == true
            then
                if mutationName == "Common" then
                    return "No Mutation"
                end

                return upgradeMutationDisplayName(mutationName)
            end
        end
    end
...
local function getUpgradeFilterSignature()
    local parts = {}
    ...
    for _, rarityName in ipairs(UPGRADE_RARITY_OPTIONS) do
        if rarityName ~= "All" then
            table.insert(
                parts,
                selectedUpgradeRarities[rarityName] == true and "1" or "0"
            )
        end
    end
    ...
    for _, mutationName in ipairs(UPGRADE_MUTATION_OPTIONS) do
        if mutationName ~= "All" then
            table.insert(
                parts,
                selectedUpgradeMutations[mutationName] == true and "1" or "0"
            )
        end
    end
...
local upgradeRarityItems = {}
local upgradeMutationItems = {}

local function setAllUpgradeOptions(selectedSet, options, enabled)
    for _, option in ipairs(options) do
        if option ~= "All" then
            selectedSet[option] = enabled == true
        end
    end
end

local function optionIsChecked(selectedSet, options, option)
    if option == "All" then
        local selectedCount, totalCount =
            countUpgradeSelections(selectedSet, options)
...
local function refreshUpgradeFilterVisuals()
    local rarityArrow =
        UpgradeRarityDropList.Visible and "▲ " or "▼ "

    local mutationArrow =
        UpgradeMutationDropList.Visible and "▲ " or "▼ "

    UpgradeRarityDropBtn.Text =
        "Rarity: "
        .. rarityArrow
        .. upgradeRaritySelectionLabel()

    UpgradeMutationDropBtn.Text =
        "Mutation: "
        .. mutationArrow
        .. upgradeMutationSelectionLabel()

    for option, item in pairs(upgradeRarityItems) do
        local checked =
            optionIsChecked(
                selectedUpgradeRarities,
                UPGRADE_RARITY_OPTIONS,
        ...
        local label =
            option == "All"
            and "Select All"
            or upgradeRarityDisplayName(option)
        ...
        item.TextColor3 =
            checked
            and Color3.fromRGB(150, 220, 255)
            or Color3.fromRGB(220, 232, 245)
    end

    for option, item in pairs(upgradeMutationItems) do
        local checked =
            optionIsChecked(
                selectedUpgradeMutations,
                UPGRADE_MUTATION_OPTIONS,
        ...
        local label =
            option == "All"
            and "Select All"
            or upgradeMutationDisplayName(option)
        ...
        item.TextColor3 =
            checked
            and Color3.fromRGB(235, 190, 255)
            or Color3.fromRGB(235, 220, 248)
    end
end

local function updateUpgradeFilterStatus()
    StatusLabel.Text =
        "Auto Upgrade | Rarity: "
        .. upgradeRaritySelectionLabel()
        .. " | Mutation: "
...
for i, rarityName in ipairs(UPGRADE_RARITY_OPTIONS) do
    ...
    upgradeRarityItems[rarityName] = item

    item.MouseButton1Click:Connect(function()
        if rarityName == "All" then
            setAllUpgradeOptions(
                selectedUpgradeRarities,
                UPGRADE_RARITY_OPTIONS,
                not allUpgradeRaritiesSelected()
            )
        else
            selectedUpgradeRarities[rarityName] =
                not (selectedUpgradeRarities[rarityName] == true)
        end

        refreshUpgradeFilterVisuals()
        updateUpgradeFilterStatus()
    end)
end
...
for i, mutationName in ipairs(UPGRADE_MUTATION_OPTIONS) do
    ...
    upgradeMutationItems[mutationName] = item
    ...
    item.MouseButton1Click:Connect(function()
        if mutationName == "All" then
            setAllUpgradeOptions(
                selectedUpgradeMutations,
                UPGRADE_MUTATION_OPTIONS,
                not allUpgradeMutationsSelected()
            )
        else
            selectedUpgradeMutations[mutationName] =
                not (selectedUpgradeMutations[mutationName] == true)
        end
...
-- Start with every option checked, which is equivalent to the old "All".
refreshUpgradeFilterVisuals()
...
-- Prevent the Lucky Block controls from sitting over the Auto Upgrade
-- multi-select lists. They are temporarily hidden only while either
-- Auto Upgrade dropdown is open, then restored immediately when it closes.
local function syncLuckyControlsWithUpgradeDropdowns()
    local upgradeDropdownOpen =
        UpgradeRarityDropList.Visible
        or UpgradeMutationDropList.Visible

    LuckyBtn.Visible = not upgradeDropdownOpen
    LuckyTypeDropBtn.Visible = not upgradeDropdownOpen
end

UpgradeRarityDropList:GetPropertyChangedSignal("Visible"):Connect(function()
    syncLuckyControlsWithUpgradeDropdowns()
end)

UpgradeMutationDropList:GetPropertyChangedSignal("Visible"):Connect(function()
    syncLuckyControlsWithUpgradeDropdowns()
end)
...
local function candidateMatchesUpgradeFilters(info)
    ...
    -- RARITY multi-select:
    -- All checked = any rarity (same behavior as the old "All").
    -- Otherwise candidate rarity must be one of the checked rarities.
    if not allUpgradeRaritiesSelected() then
        local candidateRarity =
            string.lower(
                normalizeUpgradeRarity(info.rarity)
            )

        local rarityMatched = false

        for rarityName, enabled in pairs(selectedUpgradeRarities) do
            if enabled == true then
                local wantedRarity =
                    string.lower(
                        normalizeUpgradeRarity(rarityName)
                    )
    ...
    -- All checked = any base/event mutation.
    -- Otherwise a slime is eligible when ANY checked mutation matches.
    -- "Common" specifically means truly no mutation at all.
    if allUpgradeMutationsSelected() then
        return true
    end

    for mutationName, enabled in pairs(selectedUpgradeMutations) do
        if enabled == true
            and upgradeMutationMatches(
                mutationName,
                info.mutation,
...
task.spawn(function()
    while true do
        if upgradeEnabled then
            local ok, err = xpcall(function()
                local filterSignatureAtDecision =
                    getUpgradeFilterSignature()

                local rarityLabelAtDecision =
                    upgradeRaritySelectionLabel()

                local mutationLabelAtDecision =
                    upgradeMutationSelectionLabel()

                local upgrades, stats = getPrioritizedUpgrades()

                if filterSignatureAtDecision ~= getUpgradeFilterSignature() then
                    return
                end
                ...
                if #upgrades == 0 then
                    StatusLabel.Text = string.format(
                        ...
                        rarityLabelAtDecision,
                        mutationLabelAtDecision,
                ...
                print(
                    "[AutoUpgrade] BATCH",
                    #batch,
                    "| Rarity:", rarityLabelAtDecision,
                    "| Mutation:", mutationLabelAtDecision,
                    "| Matches:", #upgrades
                )
