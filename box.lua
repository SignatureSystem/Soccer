...
-- Icons server-hop collector. Only active while Lucky Block is ON and
-- Lucky Type is Icons. The queued loader points back to the user's GitHub raw soccer.lua, so upload this generated build there before use.
local ICON_SERVER_HOP_JOIN_GRACE = 8.0
local ICON_SERVER_HOP_EMPTY_CONFIRM = 4.0
local ICON_SERVER_HOP_PAGES = 3
local ICON_SERVER_HOP_LIMIT = 50
local ICON_SERVER_HOP_RELOAD_URL =
    "https://raw.githubusercontent.com/SignatureSystem/Soccer/refs/heads/main/soccer.lua"
...
local IconHop = {
    ...
    joinedAt = os.clock(),
    visited = {},
    visitedFile = "Soccer_IconHop_" .. tostring(game.PlaceId) .. ".json",
}
...
-- ICONS SERVER HOP
-- ============================================
function IconHop.loadVisited()
    IconHop.visited = IconHop.visited or {}
    ...
    local ok, exists = pcall(function()
        return isfile(IconHop.visitedFile)
    end)
    ...
    local readOk, raw = pcall(function()
        return readfile(IconHop.visitedFile)
    end)
    ...
    local created = tonumber(payload.created) or 0
    if created > 0 and (os.time() - created) > 7200 then
        return
    end

    if type(payload.ids) == "table" then
        for _, id in ipairs(payload.ids) do
            IconHop.visited[tostring(id)] = true
        end
    end
end

function IconHop.saveVisited()
    if type(writefile) ~= "function" then
        return
    end

    local ids = {}
    for id, seen in pairs(IconHop.visited or {}) do
        if seen then
    ...
    if ok and encoded then
        pcall(function()
            writefile(IconHop.visitedFile, encoded)
        end)
    end
end

function IconHop.markVisited(jobId)
    if jobId and tostring(jobId) ~= "" then
        IconHop.visited[tostring(jobId)] = true
        IconHop.saveVisited()
    end
end

function IconHop.getQueueFunction()
...
function IconHop.queueReload()
    local queueFn = IconHop.getQueueFunction()
    if not queueFn then
        warn("[IconHop] queue_on_teleport unavailable; hop can occur but auto-resume is not guaranteed")
        return false
    end
...
pcall(function()
    if type(getgenv) == "function" then
        getgenv().SOCCER_ICON_HOP_AUTOSTART = true
    else
        _G.SOCCER_ICON_HOP_AUTOSTART = true
    end
end)
loadstring(game:HttpGet(%q))()
]=], ICON_SERVER_HOP_RELOAD_URL)
    ...
    if not ok then
        warn("[IconHop] queue_on_teleport failed:", err)
    end

    return ok
end

function IconHop.fetchServers()
    local candidates = {}
    local cursor = nil

    for _ = 1, ICON_SERVER_HOP_PAGES do
        local url = string.format(
            "https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=Asc&limit=%d",
            tostring(game.PlaceId),
            ICON_SERVER_HOP_LIMIT
        )
...
function IconHop.hop()
    if IconHop.busy then
        return false
    end

    if not luckyEnabled or selectedLuckyBlockType ~= "Icons" then
        return false
    end

    IconHop.busy = true
    IconHop.markVisited(game.JobId)

    StatusLabel.Text = "No Icons Lucky Box -> finding another server..."

    local candidates = IconHop.fetchServers()

    -- If every fetched server was already visited, clear the session list and
    -- take a fresh pass instead of getting stuck forever.
    if #candidates == 0 then
        IconHop.visited = { [tostring(game.JobId)] = true }
        IconHop.saveVisited()
        candidates = IconHop.fetchServers()
    end

    if #candidates == 0 then
        StatusLabel.Text = "Icon Hop: no open public server found; retrying..."
        IconHop.busy = false
        return false
    end

    -- Randomize the open-server choice so repeated runs do not walk the same
    -- deterministic server ordering.
    local selected = candidates[math.random(1, #candidates)]
    IconHop.markVisited(selected.id)
    ...
    StatusLabel.Text = string.format(
        "Icon Hop -> server %s (%d/%d)",
        string.sub(selected.id, 1, 8),
        selected.playing,
        selected.maxPlayers
    )
    ...
    if not ok then
        warn("[IconHop] Teleport failed:", err)
        IconHop.busy = false
        return false
    end
...
task.spawn(function()
    while true do
        if luckyEnabled and not luckyBlockBusy then
            ...
            if not block then
                if selectedLuckyBlockType == "Icons" then
                    local joinedFor = os.clock() - IconHop.joinedAt

                    if joinedFor >= ICON_SERVER_HOP_JOIN_GRACE then
                        if not IconHop.missingSince then
                            IconHop.missingSince = os.clock()
                        end

                        local missingFor = os.clock() - IconHop.missingSince
                        local remaining = math.max(
                            0,
                            ICON_SERVER_HOP_EMPTY_CONFIRM - missingFor
                        )

                        StatusLabel.Text = string.format(
                            "No Icons box | confirming %.1fs -> server hop",
                            remaining
                        )

                        if missingFor >= ICON_SERVER_HOP_EMPTY_CONFIRM then
                            luckyBlockBusy = false
                            IconHop.hop()
                            task.wait(1.0)
                            continue
                        ...
                        StatusLabel.Text = string.format(
                            "Icons: waiting for server load %.1fs",
                            math.max(0, ICON_SERVER_HOP_JOIN_GRACE - joinedFor)
                        )
                    end
                else
                    IconHop.missingSince = nil
                    StatusLabel.Text = string.format(
                        "No %s boxes | Total: %d",
                        selectedLuckyBlockType,
                        totalCollected
            ...
            -- An Icons target exists, so cancel the empty-server countdown.
            IconHop.missingSince = nil
...
-- Resume the Icons collector automatically after a queued server hop.
task.defer(function()
    local shouldAutoStart = false

    pcall(function()
        if type(getgenv) == "function" then
            shouldAutoStart = getgenv().SOCCER_ICON_HOP_AUTOSTART == true
            getgenv().SOCCER_ICON_HOP_AUTOSTART = nil
        else
            shouldAutoStart = _G.SOCCER_ICON_HOP_AUTOSTART == true
            _G.SOCCER_ICON_HOP_AUTOSTART = nil
        end
    end)

    if shouldAutoStart then
        selectedLuckyBlockType = "Icons"
        LuckyTypeDropBtn.Text = "Lucky Type: ▼  Icons"
        IconHop.joinedAt = os.clock()
        IconHop.missingSince = nil
        setLuckyState(true)
        StatusLabel.Text = "Icon Hop resumed | scanning for Icons Lucky Box..."
    end
end)
